...
func (p *Pipeline) GenerateSheets(ctx context.Context, manifest *model.ProjectManifest, note *model.CharacterNote) error {
	...
	for _, char := range note.Characters {
		...
		g.Go(func() error {
			...
			if err != nil {
				errs <- fmt.Sprintf("generating sheet for %q: %v", char.ID, err)
				return err
			}
			...
			if err := storage.SavePNG(sheetPath, result.Image); err != nil {
				errs <- fmt.Sprintf("saving sheet for %q: %v", char.ID, err)
				return err
			}

			slog.Info("generated 3x2 base sheet", "character", char.Name, "id", char.ID)
			return nil
		})
	}
